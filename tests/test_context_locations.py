...
def test_lite_context_pulls_locations_from_hub() -> None:
    ...
    assert [loc.host for loc in ctx.locations] == ["workstation", "gpu"]
...
def test_render_header_uses_first_location_host() -> None:
    ...
    ctx = RepoContext(
        ...
        locations=[Location(host="workstation", path="/data", role="data")],
    )
    rendered = render_context(ctx)
    assert "Host: workstation" in rendered
...
def _slash_ctx(answers, active_host=None):
    hub = MagicMock()
    ex = _FakeExecutor()
    env = ToolEnv(hub=hub, executors={"workstation": ("u", "t"), "gpu": ("u", "t")},
...
def test_run_on_sets_active_host() -> None:
    ctx, _ex = _slash_ctx([])
    r = try_dispatch("run on workstation: ls", ctx)
