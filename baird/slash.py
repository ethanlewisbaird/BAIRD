...
_AUDIT_PROMPT_TEMPLATE = (
    "Audit the satellite **{host}** under `{path}` for projects we should "
    "register with BAIRD, and propose any natural parent/child family trees.\n\n"
    "Use the provided function-calling tools — `run_on`, `read_remote`, "
    "`list_project_locations`, `list_projects` — to enumerate the directory "
    "layout. Begin by running `find` on {host} under {path} bounded to "
    "max-depth {depth}, skipping hidden dirs and common cache dirs "
    "(`node_modules`, `__pycache__`, `.snakemake`, `.nextflow`). For each "
    "candidate directory, peek at light signals — presence of `.git`, "
    "`README.md`, `environment.yml`, `pyproject.toml`, `Snakefile`, "
    "`nextflow.config`, data subdirs — before deciding it's a project.\n\n"
    "Skip dirs already registered as a project location (check via the "
    "project-listing tools first).\n\n"
    "When you're done, give a structured report with TWO sections:\n\n"
    "**1. Suggested projects** — flat list. For each:\n"
    "  - proposed `id` (kebab-case slug) and human-readable `name`\n"
    "  - the location to register: host `{host}`, absolute path, role "
    "(`code` / `data` / `compute` / `mixed`)\n"
    "  - one-sentence justification (what's there)\n\n"
    "**2. Suggested family trees** — parent/child groupings. Only suggest a "
    "parent when the directory layout genuinely implies a shared umbrella "
    "(e.g. a top-level dir whose children are clearly co-related "
    "assays / sub-pipelines / sub-experiments, with shared README or naming "
    "pattern). For each tree:\n"
